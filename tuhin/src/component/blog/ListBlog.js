...
export default function ListBlog() {
    ...
    const deletNews = (id) => {
        axios.get(`http://localhost/tuhin/backend/blog/deleteBlog.php?id=${id}`)
            .then((res) => {
    ...
    return (
        <div className='container card card-body'>
            <div>
                <h1 className="bg-secondary text-center text-bold">blogs List</h1>

                <NavLink to={'/blog/addblog'} className="btn btn bg-primary">Add New</NavLink><br />
            </div>
                    ...
                    {data.map((d, i) => {
                        return (
                            <tr key={i}>
                                ...
                                <td>
                                    <NavLink to={`/blog/updateblog/${d.id}`}>
